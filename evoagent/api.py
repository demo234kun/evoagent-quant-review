...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/v1/evolution/status":
            ...
            return
        if path == "/v1/findings-bank":
            limit = min(int(query.get("limit", ["500"])[0]), 1000)
            rule_id = query.get("rule_id", [None])[0]
            severity = query.get("severity", [None])[0]
            repository = query.get("repository", [None])[0]
            findings = self.service.store.list_findings_bank(
                principal.tenant_id, limit=limit, rule_id=rule_id,
                severity=severity, repository=repository,
            )
            stats = self.service.store.findings_bank_stats(principal.tenant_id)
            self._send_json(200, {"findings": findings, "stats": stats})
            return
        ...
        try:
            ...
            if path == "/v1/auth/register":
                if not self.settings.auth_required:
                    self._send_json(409, {"error": "authentication is disabled"})
                    return
                if not getattr(self.settings, "open_registration", True):
                    self._send_json(403, {"error": "registration is closed"})
                    return
                payload = self._read_json(body)
                ...
                if not username or not password:
                    ...
                    return
                try:
                    result = self.service.auth.register(username, password, tenant_id)
                except PermissionError as exc:
                ...
                try:
                    self.service.store.audit(
                        result.get("tenant_id", "default"), username, "auth.register",
                        username, {"role": result.get("role")},
                    )
                except Exception:
                    pass
                return
            ...
            if path == "/v1/backtests":
                ...
                repository = str(payload.get("repository", "backtest/upload"))
                result = self.service.create_backtest(
                    repository=repository, strategy_code=strategy_code,
                    params=params, source="api", tenant_id=principal.tenant_id,
                )
                self.service.store.audit(
                    principal.tenant_id, principal.username, "backtest.create",
                    repository, {"symbol": params.get("symbol")},
                )
                self._send_json(201, result)
                return
