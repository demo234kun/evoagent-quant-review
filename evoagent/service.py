...
class ReviewService:
    def __init__(self, settings: Settings):
        ...
        self.market_data = MarketDataProvider(settings.backtest_data_dir)
        self.backtest_harness = BacktestHarness(
            self.store, self.reviewer, settings, self.market_data
        )
    ...
    def create_review(
        ...
        try:
            ...
            self._ingest_findings(task_id, repository, report, tenant_id)
    ...
    def create_backtest(
        self, repository: str, strategy_code: str, params: Dict[str, Any],
        source: str = "api", tenant_id: str = "default",
    ) -> Dict[str, Any]:
        code_bytes = (strategy_code or "").encode("utf-8")
        if len(code_bytes) == 0:
            raise ValueError("strategy code is required")
        if len(code_bytes) > self.settings.max_diff_bytes:
            raise ValueError(
                "strategy code exceeds maximum size of %d bytes"
                % self.settings.max_diff_bytes
            )
        task_id = str(uuid.uuid4())
        input_json = {
            "source": source, "type": "backtest", "mode": "rules-only",
            "repository": repository, "params": params,
        }
        self.store.create(task_id, repository, None, input_json, tenant_id)
        self.store.save_task_payload(task_id, strategy_code)
        try:
            with self.observability.span(
                "backtest", task_id, task_id=task_id, tenant_id=tenant_id,
                repository=repository,
            ), metrics.timer("backtest_duration"):
                report = self.backtest_harness.run(
                    task_id, repository, strategy_code, params, tenant_id
                )
            metrics.inc("backtests_total")
            self._ingest_findings(task_id, repository, report, tenant_id)
            return {"task_id": task_id, "state": "SUCCESS", "report": report.to_dict()}
        except Exception as exc:
            self.store.fail(
                task_id, str(exc)[:2000],
                TraceEvent(1, TaskState.FAILED, "Backtest failed", utc_now()),
            )
            self.alerts.evaluate(tenant_id)
            raise

    def _ingest_findings(
        self, task_id: str, repository: str, report: Any, tenant_id: str
    ) -> None:
        """Persist every finding from a review/backtest report into the memory bank."""
        rep = report.to_dict() if hasattr(report, "to_dict") else report
        if not isinstance(rep, dict):
            return
        findings = rep.get("findings") or []
        for finding in findings:
            self.store.add_finding(tenant_id, {
                "task_id": task_id,
                "repository": repository,
