...
class PlannerAgent:
    ...
    DOMAIN_OBJECTIVES = {
        "security": "查找量化代码中的前视偏差（未来函数）以及数据泄露 / 幸存者偏差风险。",
        "quant-future-function": "查找读取未来价格、K线或时间戳的前视偏差（未来函数）。",
        "quant-data-leakage": "查找由全样本统计或时点违规导致的数据泄露与幸存者偏差。",
        "reliability": "查找量化代码中的执行 / 成本假设、过拟合与数据质量回退。",
        "quant-execution": "查找回测中的零手续费 / 零滑点假设与不现实成交价。",
        "quant-overfitting": "查找缺少滚动前进或剔除时序交叉验证的样本内参数优化。",
        "quant-data-quality": "查找复权 / fq 不匹配、NaN/inf 处理与数值精度错误。",
    }
...
class EvidenceAgent:
    ...
    def reproduce(self, finding: Finding, parsed: ParsedDiff) -> Reproduction:
        ...
        signatures = {
            "QUANT-FF-SHIFT-NEG": ".shift(-" in normalized,
            "QUANT-FF-REF-NEG": "ref(" in line and "-1" in line,
            "QUANT-FF-DATETIME-NOW": "datetime.now(" in line or "datetime.today(" in line,
            "QUANT-FF-GETPRICE-NOW": (
                ("get_price(" in line or "attribute_history(" in line)
                and "current_dt" in line
            ),
            "QUANT-FF-CHEAT-CLOSE": "set_coc(true" in normalized,
            "QUANT-LK-FULL-NORM": "fit_transform(" in line,
            "QUANT-LK-FULL-ZSCORE": ".mean(" in line and ".std(" in line,
            "QUANT-LK-FORWARD-FILL": "bfill" in line,
            "QUANT-LK-STATDATE": "statdate" in line.lower(),
            "QUANT-LK-INDEX-NOW": "get_index_stocks(" in line,
            "QUANT-SEC-HARDCODED-KEY": any(
                token in line.lower() for token in ("password", "secret", "token", "api_key")
            ),
            "QUANT-SEC-NO-RISK": "order_target_percent" in line and "1.0" in line,
            "QUANT-EX-ZERO-SLIP": "slip_perc=0" in normalized,
            "QUANT-EX-ZERO-COMM": "commission=0" in normalized,
            "QUANT-OF-OPTSTRATEGY": "optstrategy(" in line,
            "QUANT-OF-GRIDSEARCH": "gridsearchcv(" in normalized,
            "QUANT-DQ-ADJUST-POST": "fq='post'" in line or 'fq="post"' in line,
            "QUANT-DQ-FILLNA0": "fillna(0" in normalized,
            "QUANT-REL-DEBUG-PRINT": "print(" in line or "console.log(" in line,
        }
        ...
        return Reproduction(
            ...
            "独立的变更行证据核验",
            line.strip()[:240] if reproducible else "未找到可独立匹配的变更行证据。",
        )
