...
class AuthManager:
    ...
    def register(
        self, username: str, password: str, tenant_id: str = "", role: str = "maintainer",
    ) -> Dict[str, object]:
        username = (username or "").strip()
        if not username:
            raise ValueError("username is required")
        if self.store.get_user(username) is not None:
            raise PermissionError("username already registered")
        tenant = (tenant_id or "").strip() or self.default_tenant_id
        password_hash = hash_password(password)
        user_id = str(uuid.uuid5(uuid.NAMESPACE_URL, "evoagent:" + username))
        self.store.create_user(user_id, username, password_hash, tenant, role)
        return self.login(username, password, tenant)
