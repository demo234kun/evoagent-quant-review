...
def generate_prompt_evolution_cases() -> List[dict]:
    ...
    baseline_risks = [
        (
            "QUANT-FF-SHIFT-NEG", "critical",
            lambda n: "signal_%s = df['close'].shift(-%s)" % (n, n),
        ),
        (
            "QUANT-FF-DATETIME-NOW", "high",
            lambda n: "now_%s = datetime.now()" % n,
        ),
        (
            "QUANT-LK-FULL-NORM", "high",
            lambda n: "scaled_%s = scaler.fit_transform(data_%s)" % (n, n),
        ),
        (
            "QUANT-REL-DEBUG-PRINT", "low",
            lambda n: "print('debug signal', signal_%s)" % n,
        ),
    ]
    adaptive_risks = [
        (
            "QUANT-FF-REF-NEG", "critical",
            lambda n: "value_%s = ref(close, -%s)" % (n, n),
        ),
        (
            "QUANT-FF-GETPRICE-NOW", "high",
            lambda n: "prices_%s = get_price('000001.XSHE', end_date=context.current_dt)" % n,
        ),
        (
            "QUANT-LK-FORWARD-FILL", "medium",
            lambda n: "df_%s = df_%s.fillna(method='bfill')" % (n, n),
        ),
        (
            "QUANT-FF-CHEAT-CLOSE", "high",
            lambda n: "cerebro.broker.set_coc(True)",
        ),
    ]
    clean_lines = [
