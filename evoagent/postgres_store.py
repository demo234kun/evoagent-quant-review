...
class PostgresTaskStore:
    ...
    def _init(self) -> None:
        statements = [
            ...
            """CREATE TABLE IF NOT EXISTS findings_bank (
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    "ALTER TABLE findings_bank ADD COLUMN IF NOT EXISTS category TEXT DEFAULT ''"
                )
    ...
    def add_finding(self, tenant_id: str, finding: Dict[str, Any]) -> None:
        with self._connect() as conn:
            conn.execute(
                "INSERT INTO findings_bank("
                "tenant_id, task_id, repository, rule_id, severity, title, "
                "explanation, path, line, evidence, strategy_name, category, created_at) "
                ...
                (
                    ...
                    finding.get("strategy_name"),
                    finding.get("category") or "",
                    utc_now(),
                ),
            )

    def list_findings_bank(
        self, tenant_id: str, limit: int = 200,
        rule_id: Optional[str] = None, severity: Optional[str] = None,
        ...
        query = (
            "SELECT * FROM findings_bank WHERE " + " AND ".join(clauses)
        ...
        for value in values:
            ca = value.get("created_at")
            if hasattr(ca, "isoformat"):
                value["created_at"] = ca.isoformat()
        return values

    def findings_bank_stats(self, tenant_id: str) -> Dict[str, Any]:
        with self._connect() as conn:
            total = conn.execute(
                "SELECT COUNT(*) AS c FROM findings_bank WHERE tenant_id=%s",
                (tenant_id,),
            ).fetchone()["c"]
            sev = conn.execute(
                "SELECT severity, COUNT(*) AS c FROM findings_bank "
                "WHERE tenant_id=%s GROUP BY severity",
                (tenant_id,),
            ).fetchall()
            rules = conn.execute(
                "SELECT rule_id, COUNT(*) AS c FROM findings_bank "
                "WHERE tenant_id=%s GROUP BY rule_id ORDER BY c DESC",
                (tenant_id,),
            ).fetchall()
            strategies = conn.execute(
                "SELECT COUNT(DISTINCT path) AS c FROM findings_bank "
                "WHERE tenant_id=%s",
                (tenant_id,),
            ).fetchone()["c"]
        return {
            "total": total,
            "by_severity": {r["severity"]: r["c"] for r in sev},
            "by_rule": [{"rule_id": r["rule_id"], "count": r["c"]} for r in rules],
            "strategies_scanned": strategies,
        }
