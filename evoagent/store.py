...
class TaskStore:
    ...
    def _init(self) -> None:
        with self._connect() as conn:
            ...
            self._ensure_column(conn, "findings_bank", "category", "TEXT DEFAULT ''")
    ...
    def findings_bank_stats(self, tenant_id: str) -> Dict[str, Any]:
        with self._connect() as conn:
            total = conn.execute(
                "SELECT COUNT(*) AS c FROM findings_bank WHERE tenant_id = ?",
                (tenant_id,),
            ).fetchone()["c"]
            sev = conn.execute(
            ...
            rules = conn.execute(
                ...
                "WHERE tenant_id = ? GROUP BY rule_id ORDER BY c DESC",
                (tenant_id,),
            ).fetchall()
