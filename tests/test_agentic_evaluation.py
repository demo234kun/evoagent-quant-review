...
class AgenticEvaluationTests(unittest.TestCase):
    def test_all_arms_share_exactly_fourteen_rules_and_real_role_topologies(self):
        self.assertEqual(22, len(LocalRuleReviewer.RULES) + len(ContextRuleReviewer.RULES))
        expected_calls = {
            "rules-only": {},
        ...
        for arm, calls in expected_calls.items():
            ...
            self.assertEqual(["QUANT-FF-SHIFT-NEG"], [item.rule_id for item in findings])
            actual = {}
        ...
        for index, split in enumerate(("train", "validation", "holdout"), 1):
            cases.append({
                ...
                "expected_findings": [{
                    ...
                    "rule_id": "QUANT-FF-SHIFT-NEG", "cwe": "QUANT-FF-SHIFT-NEG",
