...
class MultiAgentCollaborationTests(unittest.TestCase):
    ...
    def test_security_and_reliability_are_independent_specialists(self):
        ...
        self.assertEqual({"QUANT-FF-SHIFT-NEG"}, {item.rule_id for item in security})
        self.assertEqual({"QUANT-REL-DEBUG-PRINT"}, {item.rule_id for item in reliability})
    ...
    def test_failed_agent_is_retried_then_replanned_to_substitute(self):
        ...
        self.assertEqual({"QUANT-REL-DEBUG-PRINT"}, {item.rule_id for item in findings})
