...
class EndToEndEvaluationTests(unittest.TestCase):
    def test_generated_dataset_has_repository_level_split_and_expected_counts(self):
        ...
        self.assertEqual(201, len(cases))
        self.assertEqual(81, sum(bool(item["expected_findings"]) for item in cases))
        self.assertEqual(120, sum(not item["expected_findings"] for item in cases))
        validation_repos = {
