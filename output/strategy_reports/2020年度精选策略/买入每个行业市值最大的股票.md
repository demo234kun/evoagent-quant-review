# EvoAgent 量化代码审查

**Repository:** `聚宽2020/买入每个行业市值最大的股票`  
**Risk:** `medium`  
**Reviewer:** `mode-router`

已审查 1 个文件；发现 3 个可处理问题。整体风险：medium。

## 执行概况

- 模式：请求 `rules-only`，实际 `rules-only`
- 模型调用: `0`；工具调用: `4`
- Token：输入 `0`、输出 `0`、合计 `0`
- Token 成本: `$0.00000000`；延迟: `304 ms`

## 审查发现

### 1. 🟠 使用 statDate 拉取财报期数据，可能包含未披露数据

`买入每个行业市值最大的股票.py:301` · **MEDIUM** · `QUANT-LK-STATDATE`

用 statDate 指定报告期会在报告实际发布前取到数据，形成前视。

**证据**

```text
df = get_fundamentals(q, statDate = myDate).fillna(value=0)
```

**证据引用：** `local-rule:b7e054f78f27a093`

**修复建议：** 用 point-in-time 的发布日字段，或在回测日之前取最近已披露财报。

**测试建议：** 断言取到的财报披露日不晚于回测当前日。

### 2. 🟠 假设零手续费（set_commission 缺失/为 0）

`买入每个行业市值最大的股票.py:990` · **MEDIUM** · `QUANT-EX-ZERO-COMM`

手续费为 0 会高估高频/换手策略收益。

**证据**

```text
set_order_cost(OrderCost(open_tax=0, close_tax=0, open_commission=0, close_commission=0, close_today_commission=0, min_commission=0), type='fund')
```

**证据引用：** `local-rule:c01dfeea9838d381`

**修复建议：** 设置与券商一致的佣金、印花税与最低手续费。

**测试建议：** 断言手续费计入后收益曲线符合预期。

### 3. 🟠 假设零手续费（set_commission 缺失/为 0）

`买入每个行业市值最大的股票.py:992` · **MEDIUM** · `QUANT-EX-ZERO-COMM`

手续费为 0 会高估高频/换手策略收益。

**证据**

```text
set_order_cost(OrderCost(open_tax=0, close_tax=0.001, open_commission=0.0003, close_commission=0.0003, close_today_commission=0, min_commission=5), type='stock')
```

**证据引用：** `local-rule:6060b1509b3fa772`

**修复建议：** 设置与券商一致的佣金、印花税与最低手续费。

**测试建议：** 断言手续费计入后收益曲线符合预期。

