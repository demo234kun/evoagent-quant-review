# EvoAgent 量化代码审查

**Repository:** `聚宽2025/动态保形回归小市值策略改进`  
**Risk:** `high`  
**Reviewer:** `mode-router`

已审查 1 个文件；发现 4 个可处理问题。整体风险：high。

## 执行概况

- 模式：请求 `rules-only`，实际 `rules-only`
- 模型调用: `0`；工具调用: `6`
- Token：输入 `0`、输出 `0`、合计 `0`
- Token 成本: `$0.00000000`；延迟: `256 ms`

## 审查发现

### 1. 🔴 get_price 未指定结束日期，默认拉取当前时刻数据

`动态保形回归小市值策略改进.py:399` · **HIGH** · `QUANT-FF-GETPRICE-NO-ENDDATE`

get_price 不传 end_date 时默认取到当前时刻，回测中等于使用了尚未发生的 bar，属于前视。

**证据**

```text
df['close1']=list(get_price(dfindex, end_date=today,count =1, fq='pre',panel=False)['close'])
```

**证据引用：** `local-rule:3e0093a7ed78ca87`, `diff-ast:ab5212a917352ca4`

**修复建议：** 显式传 end_date=context.previous_date 或使用 history()。

**测试建议：** 断言行情查询始终带有明确的结束日期且不晚于当前信号 bar。

### 2. 🔴 get_price 未指定结束日期，默认拉取当前时刻数据

`动态保形回归小市值策略改进.py:400` · **HIGH** · `QUANT-FF-GETPRICE-NO-ENDDATE`

get_price 不传 end_date 时默认取到当前时刻，回测中等于使用了尚未发生的 bar，属于前视。

**证据**

```text
df['close2']=list(get_price(dfindex,end_date=preday,count=1,fq='pre',panel=False)['close'])
```

**证据引用：** `local-rule:0e5118a6f60519bf`, `diff-ast:94b0bd12cc5745bc`

**修复建议：** 显式传 end_date=context.previous_date 或使用 history()。

**测试建议：** 断言行情查询始终带有明确的结束日期且不晚于当前信号 bar。

### 3. 🟠 取指数成分股未指定 date，默认当前成分（幸存者偏差）

`动态保形回归小市值策略改进.py:77` · **MEDIUM** · `QUANT-LK-INDEX-NOW`

不传 date 时取到的是当前成分股，已剔除退市/调出股票，回测会幸存者偏差。

**证据**

```text
# stock_list = get_index_stocks(MKT_index)
```

**证据引用：** `local-rule:cb88f091c13cfcaa`

**修复建议：** 传入回测对应的历史 date 参数获取当时成分股。

**测试建议：** 对比传入历史 date 与默认结果的成分股数量与标的差异。

### 4. 🟡 用 0 填补缺失值可能扭曲收益率

`动态保形回归小市值策略改进.py:234` · **LOW** · `QUANT-DQ-FILLNA0`

价格/因子缺失填 0 会产生异常收益率并污染信号。

**证据**

```text
bons_df = bons_df.fillna(0)
```

**证据引用：** `local-rule:842f5249018f8956`

**修复建议：** 用 ffill、插值或丢弃缺失，并在缺失处理时只用历史信息。

**测试建议：** 构造含缺失的用例，断言缺失行未被填成不合理数值。

